...
"""
DentaBot – Python FastAPI Chatbot Microservice
================================================
Runs on port 5001 (separate from the Node.js backend on 4000).

Flow:
  1. Receive message + hospital_id + conversation history
  2. Detect intent → run targeted PostgreSQL queries
  3. Decrypt sensitive fields (AES-256-GCM, same key as Node backend)
  4. Inject DB results as context into Gemini prompt
  5. If DB has no data, Gemini answers from general dental knowledge
...
# ──────────────────────────────────────────
# DB connection pool (shared across requests)
# ──────────────────────────────────────────
...
app = FastAPI(title="DentaBot", version="1.0.0", lifespan=lifespan)
...
_INTENTS = {
    ...
    "today": re.compile(r"today|tonight|today's", re.I),
    # ... your other intents
...
# ──────────────────────────────────────────
# DB context fetcher (hospital-scoped)
# ──────────────────────────────────────────
async def fetch_db_context(message: str, hospital_id: str) -> dict[str, Any]:
    intent = detect_intent(message)
    ctx: dict[str, Any] = {}
    ...
    async with _pool.acquire() as conn:  # type: ignore[union-attr]
        ...
        # Inside fetch_db_context...
        if intent["today"] or intent["appointment"]:
          rows = await conn.fetch(
        """
        SELECT a.time::text, u.name AS dentist_name, p.email AS patient_email, a.status
        FROM appointments a
        LEFT JOIN users u ON u.id = a.dentist_id
        LEFT JOIN patients p ON p.id = a.patient_id
        WHERE a.hospital_id = $1 
          AND a.date = CURRENT_DATE  -- <--- Strictly today
          AND a.deleted_at IS NULL
        ORDER BY a.time ASC
        """,
        hospital_id,
    )
        ctx["todaysSchedule"] = [dict(r) for r in rows]
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        log.warning("Could not fetch hospital name: %s", exc)

    # 2. Fetch DB context
    db_ctx = await fetch_db_context(req.message.strip(), hospital_id)
